...
- **2026-06-20** — [15 · Live news APIs (NewsAPI + GNews + Currents)](docs/15-live-news-apis.md) — Three-provider fan-out through a Next.js API route that holds keys server-side; strict category mapping (drop, don't dump in "general"); merges with mock so the app keeps working without keys; live cards open the publisher's site in a new tab.
- **2026-06-20** — [14 · Speak button on the article body](docs/14-speak-on-article-body.md) — A quick Listen pill sits above the article prose on `/article/[id]` so you can speak the full body without opening the AI Assistant Listen tab; respects the Premium paywall (only speaks the visible paragraphs).
- **2026-06-20** — [13 · Speak option inside the AI Assistant panel](docs/13-speak-on-ai-panel.md) — Summary / Bullets / Takeaways tabs each get a small Speak pill with a per-tab language chip — hear the AI summary read aloud in any of the 23 languages.
- **2026-06-20** — [12 · Speak button on every news card](docs/12-speak-button-on-news-card.md) — A per-card Speak control with a language chip; click any article on the Home/category/search/bookmark feeds to hear the headline + summary in the language you pick (all 23 supported), no need to open the article first.
- **2026-06-20** — [11 · All 22 Indian languages: speak + read](docs/11-indian-languages-speak-and-read.md) — Voice Reader gets a language-first picker, filters system voices to the chosen language, tags utterances with the right BCP-47 locale, and speaks seeded translations when available. Article `a1` now has Bengali, Tamil, Telugu, Marathi, and Gujarati translations on top of Hindi.
